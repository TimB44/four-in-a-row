    ...
    pub fn current_score(&self) -> i32 {
        if self.is_over() {
            return match self.winner(){
                Some(GamePlayer::Computer) => 10_000,
                Some(GamePlayer::Opponent) => -10_000,
                None => 0,
            }
        }
        ...
        for col in 0..7 {
            ...
            let modifier = match self.board[row - 1][col].expect("Error in current score logic") {
                GamePlayer::Opponent => -1,
                GamePlayer::Computer => 1,
            };
    ...
    pub fn build(
        ...
        board.iter().for_each(|row| {
            row.iter().for_each(|piece| match piece {
                Some(x) if *x == first_player => first_count += 1,
                Some(_) => second_count += 1,
                None => (),
            })
        });
